...
class EvalArgs:
    ...
    # Optional explicit suite-local task IDs.
    #
    # Examples:
    #   [6]                   -> evaluate only task 6
    #   [0, 1, 2, 3, 4, 5]   -> evaluate Base tasks
    #
    # None preserves the original max_tasks / all-task behavior.
...
def _validate_args(args: EvalArgs, adapter: BenchmarkAdapter) -> None:
    ...
    if args.task_ids is not None:
        if len(args.task_ids) == 0:
            raise ValueError(
                "`task_ids` was provided but is empty."
            )
        ...
        if len(set(normalized_task_ids)) != len(
            ...
            raise ValueError(
                f"`task_ids` contains duplicates: "
                f"{normalized_task_ids}"
            )
        ...
        if args.max_tasks is not None:
            raise ValueError(
                "Do not specify both `task_ids` and "
                "`max_tasks`."
            )
...
def _build_policy_example_from_obs(
    ...
    task_description: str,
    *,
    routing_gt_task_id: int | None = None,
    routing_episode_id: str | None = None,
    ...
    if routing_gt_task_id is not None:
        example_dict["routing_gt_task_id"] = int(routing_gt_task_id)
    if routing_episode_id is not None:
        example_dict["routing_episode_id"] = str(routing_episode_id)
    ...
    try:
        ...
        while t < max_steps + args.num_steps_wait:
            ...
            example_dict = {
                ...
                "lang": str(task_description),
                "state": np.expand_dims(state, axis=0).astype(np.float32, copy=False),
                "routing_gt_task_id": int(task_id),
                "routing_episode_id": f"task{int(task_id)}_ep{int(episode_idx)}",
            }
            if wrist_img is not None:
                example_dict["wrist_image"] = [wrist_img]
...
def _task_id_list(
    ...
    """
    Return suite-local task IDs to evaluate.

    If task_ids is None, preserve the original behavior:
        max_tasks=6 -> [0, 1, 2, 3, 4, 5]

    If task_ids is provided:
        task_ids=[6] -> [6]
        task_ids=[0, 2, 5] -> [0, 2, 5]
    """

    if task_ids is None:
        return list(
            range(int(num_tasks_to_run))
        )
    ...
    invalid = [
        ...
        if task_id < 0
        or task_id >= num_tasks_in_suite
    ]

    if invalid:
        raise ValueError(
            f"Invalid task IDs {invalid}. "
            f"Suite contains "
            f"{num_tasks_in_suite} tasks, "
            f"valid IDs are "
...
def _serial_eval(
    *,
    ...
    try:
        for task_id in _task_id_list(task_suite, num_tasks_to_run, args.task_ids,):
                    ...
                    if should_dispatch:
                        ...
                        examples = [
                            _build_policy_example_from_obs(
                                ...
                                routing_gt_task_id=(
                                    None
                                    if slot_states[slot_id].work_item is None
                                    else int(slot_states[slot_id].work_item.task_id)
                                ),
                                routing_episode_id=(
                                    None
                                    if slot_states[slot_id].work_item is None
                                    else f"task{int(slot_states[slot_id].work_item.task_id)}_ep{int(slot_states[slot_id].work_item.episode_idx)}"
                                ),
                            )[0]
...
def eval_libero(args: EvalArgs) -> None:
    ...
    if args.task_ids is not None:
        ...
        # Validate against the actual suite and obtain the
        # exact number of selected tasks.
        selected_task_ids = _task_id_list(
            task_suite,
            num_tasks_in_suite,
            args.task_ids,
        )

        num_tasks_to_run = len(
            selected_task_ids
        )

    elif args.max_tasks is not None:

        num_tasks_to_run = min(
            num_tasks_in_suite,
            int(args.max_tasks),
        )
    _log_suite_progress(
