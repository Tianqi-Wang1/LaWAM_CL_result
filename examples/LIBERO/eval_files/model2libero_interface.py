...
class ModelClient:
    def __init__(
        ...
        **kwargs,   
        ...
        self.server_is_routing_v1 = False
    ...
    def _validate_server_metadata_or_raise(self, *, framework_name: str) -> None:
        ...
        self.server_is_routing_v1 = bool(metadata.get("routing_v1", False))
    ...
    def step_batch(
        ...
        *,
        ...
        for idx, (example, cache_key) in enumerate(zip(examples, cache_keys)):
            ...
            # The cache slot is the stable online rollout slot used by batched LIBERO eval.
            # Pass it through as diagnostics metadata for Routing-V1.
            if bool(self.server_is_routing_v1):
                prepared["routing_slot_id"] = int(cache_key)
    ...
    def _prepare_example(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        # Optional Routing-V1 metadata is forwarded to the routing server but
        # stripped before LaWAM batch construction. Ordinary policy servers
        # simply ignore these absent keys because standard evaluation never sets them.
        if bool(self.server_is_routing_v1):
            for routing_key in ("routing_gt_task_id", "routing_episode_id", "routing_slot_id"):
                if routing_key in example:
                    inference_example[routing_key] = example[routing_key]
