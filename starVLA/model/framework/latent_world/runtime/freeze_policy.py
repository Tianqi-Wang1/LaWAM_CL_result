...
def _apply_routing_v2_memory_training(
    ...
    dynamics_only = bool(
        getattr(policy_backend.model_cfg, "routing_v2_memory_dynamics_only", False)
    )
    if dynamics_only:
        # 2x3 ablation: reuse the existing Semantic bank and train only the new
        # future-dynamics verifier. This also avoids storing six Semantic AEs.
        memory.requires_grad_(False)
        memory.dynamics.requires_grad_(True)
        allowed_prefix = "routing_v2_memory.dynamics."
    else:
        memory.requires_grad_(True)
        allowed_prefix = "routing_v2_memory."

    trainable = [(n, p) for n, p in policy_backend.named_parameters() if p.requires_grad]
    unexpected = [n for n, _ in trainable if not n.startswith(allowed_prefix)]
    if unexpected:
        raise RuntimeError(
            "Routing-V2 memory isolation failed; unexpected trainable tensors: "
            f"{unexpected[:50]}"
        )
