...
from .conditioning_adapter import BottleneckProjectionAdapter
...
class VectorMLP(nn.Module):
    ...
    def __init__(
        ...
        hidden_dim: int,
        *,
        conditioning_adapter_bottleneck: int = 0,
        conditioning_adapter_zero_init: bool = True,
    ) -> None:
        super().__init__()
        # Keep the original state-dict keys (net.0 / net.1) for checkpoint compatibility.
        self.net = nn.Sequential(
            nn.Linear(in_dim, hidden_dim),
            nn.LayerNorm(hidden_dim),
        )
        bottleneck = int(conditioning_adapter_bottleneck)
        self.conditioning_adapter = (
            BottleneckProjectionAdapter(
                in_dim,
                hidden_dim,
                bottleneck,
                zero_init=bool(conditioning_adapter_zero_init),
            )
            if bottleneck > 0
            else None
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.dim() < 3:
            x = x.unsqueeze(1)
        projected = self.net[0](x)
        if self.conditioning_adapter is not None:
            projected = projected + self.conditioning_adapter(x)
        return self.net[1](projected)
...
class ExpertLatentPredictionHead(nn.Module):
    """Predict one chunk-level latent action from final Action-Expert hidden states."""
    ...
    def __init__(
        ...
        *,
        ...
        if input_dim <= 0 or hidden_dim <= 0 or output_dim <= 0:
            raise ValueError(
                "ExpertLatentPredictionHead dimensions must be positive, got "
...
class ConditionalFlowMatchingConfig:
    ...
    # Optional task-specific nonlinear residual experts attached to the final
    # DiT blocks. A value of 0 preserves the original architecture exactly.
    residual_expert_num_blocks: int = 0
    # Optional explicit DiT layer placement, e.g. [3, 7, 11, 15].
    # When set, this overrides final-N placement.
    residual_expert_layer_indices: Optional[Tuple[int, ...]] = None
    residual_expert_zero_init: bool = True
    residual_expert_scale: float = 1.0

    # CLARE-style task conditioning adapters. A value of 0 disables them.
    conditioning_adapter_bottleneck: int = 0
    conditioning_adapter_target_enc_vlm: bool = True
    conditioning_adapter_target_adanorm: bool = True
    conditioning_adapter_zero_init: bool = True

    # Activation-space nonlinear side adapters attached in parallel to the
    # frozen Attention/FFN sublayers of selected DiT blocks.  A value of 0
    # disables them.
    dit_nonlinear_adapter_bottleneck: int = 0
    dit_nonlinear_adapter_layer_indices: Optional[Tuple[int, ...]] = None
    dit_nonlinear_adapter_target_attention: bool = True
    dit_nonlinear_adapter_target_ffn: bool = True
    dit_nonlinear_adapter_zero_init: bool = True

    # Routing-V1 auxiliary branch.  When enabled, the final DiT action-token
    # hidden states are mask-mean pooled and mapped to one LAM latent action z*.
    # `expert_latent_dim` is auto-aligned to the loaded LAM code dimension by
    # LatentWorldPolicyBackend before this Flow head is instantiated.
    enable_expert_latent_head: bool = False
    expert_latent_dim: int = -1
    expert_latent_head_hidden_dim: int = 1024
    expert_latent_head_dropout: float = 0.0
...
class ConditionalFlowMatchingHead(nn.Module):
    def __init__(self, config: Optional[ConditionalFlowMatchingConfig] = None):
        ...
        # )
        enc_adapter_bottleneck = (
            int(self.config.conditioning_adapter_bottleneck)
            if bool(self.config.conditioning_adapter_target_enc_vlm)
            else 0
        )
        self.enc_vlm = VectorMLP(
            in_dim=self.config.vlm_dim,
            hidden_dim=self.config.vision_dim,
            conditioning_adapter_bottleneck=enc_adapter_bottleneck,
            conditioning_adapter_zero_init=bool(self.config.conditioning_adapter_zero_init),
        )
        ...
        if bool(self.config.enable_expert_latent_head):
            latent_dim = int(self.config.expert_latent_dim)
            if latent_dim <= 0:
                raise ValueError(
                    "enable_expert_latent_head=True requires expert_latent_dim > 0. "
                    "LatentWorldPolicyBackend normally auto-aligns this to lam.code_dim."
                )
            self.expert_latent_head: Optional[ExpertLatentPredictionHead] = ExpertLatentPredictionHead(
                input_dim=int(self.config.hidden_dim),
                hidden_dim=int(self.config.expert_latent_head_hidden_dim),
                output_dim=latent_dim,
                dropout=float(self.config.expert_latent_head_dropout),
            )
        else:
            self.expert_latent_head = None
        ...
        dit_kwargs = {
            ...
            "residual_expert_num_blocks": self.config.residual_expert_num_blocks,
            "residual_expert_layer_indices": (
                None
                if self.config.residual_expert_layer_indices is None
                else [int(x) for x in self.config.residual_expert_layer_indices]
            ),
            "residual_expert_zero_init": self.config.residual_expert_zero_init,
            "residual_expert_scale": self.config.residual_expert_scale,
            "conditioning_adapter_bottleneck": (
                int(self.config.conditioning_adapter_bottleneck)
                if bool(self.config.conditioning_adapter_target_adanorm)
                else 0
            ),
            "conditioning_adapter_zero_init": bool(self.config.conditioning_adapter_zero_init),
            "dit_nonlinear_adapter_bottleneck": int(self.config.dit_nonlinear_adapter_bottleneck),
            "dit_nonlinear_adapter_layer_indices": (
                None
                if self.config.dit_nonlinear_adapter_layer_indices is None
                else [int(x) for x in self.config.dit_nonlinear_adapter_layer_indices]
            ),
            "dit_nonlinear_adapter_target_attention": bool(self.config.dit_nonlinear_adapter_target_attention),
            "dit_nonlinear_adapter_target_ffn": bool(self.config.dit_nonlinear_adapter_target_ffn),
            "dit_nonlinear_adapter_zero_init": bool(self.config.dit_nonlinear_adapter_zero_init),
        }
    ...
    def _predict_expert_latent(
        ...
        *,
        ...
        """Return z* with shape [B, 1, D_z] from the final action-token hidden states."""
        if self.expert_latent_head is None:
            raise RuntimeError(
                "Expert latent prediction requested but `expert_latent_head` is disabled. "
                "Set framework.action_model.flow_cfg.enable_expert_latent_head=true."
            )
        if action_seq_len <= 0:
            raise ValueError(f"action_seq_len must be > 0, got {action_seq_len}.")
        action_hidden = dit_output[:, -int(action_seq_len) :, :]
        if token_valid.shape != action_hidden.shape[:2]:
            raise ValueError(
                "Expert latent pooling mask mismatch: "
                f"token_valid={tuple(token_valid.shape)}, action_hidden={tuple(action_hidden.shape)}."
            )
        mask = token_valid.to(device=action_hidden.device, dtype=action_hidden.dtype).unsqueeze(-1)
        denom = mask.sum(dim=1).clamp_min(1.0)
        pooled = (action_hidden * mask).sum(dim=1) / denom
        latent = self.expert_latent_head(pooled)
    ...
    def forward(
        ...
        return_expert_latent: bool = False,
        ...
        expert_latent = None
        if bool(return_expert_latent):
            expert_latent = self._predict_expert_latent(
                dit_output=dit_output,
                token_valid=token_valid,
                action_seq_len=int(actions.shape[1]),
            )
    ...
    def sample_actions_cfg(
        ...
        if initial_noise is None:
            ...
            expected_noise_shape = (batch_size, action_horizon, int(self.config.action_dim))
            if tuple(initial_noise.shape) != expected_noise_shape:
                raise ValueError(
                    "Routing initial-noise shape mismatch: "
                    f"got={tuple(initial_noise.shape)}, expected={expected_noise_shape}."
                )
            x_t = initial_noise.to(device=device, dtype=model_dtype)
        ...
        last_model_output_cond: Optional[torch.Tensor] = None
        ...
        for step in range(num_inference_steps):
            ...
            # The Routing-V1 latent head is defined on the conditional DiT hidden
            # state from the final denoising forward. Keep the latest conditional
            # hidden here; after the loop this is exactly the last inference step.
            last_model_output_cond = model_output_cond

            pred_velocity = pred_velocity * time_valid.unsqueeze(-1).to(dtype=pred_velocity.dtype)
            x_t = x_t + dt * pred_velocity
            x_t = x_t * time_valid.unsqueeze(-1).to(dtype=x_t.dtype)

        actions_out = x_t[:, :output_horizon, :]
        if not bool(return_expert_latent):
            return actions_out

        if last_model_output_cond is None:
            raise RuntimeError("Routing-V1 requested expert latent but no DiT inference step was executed.")
        expert_latent = self._predict_expert_latent(
            dit_output=last_model_output_cond,
            token_valid=time_valid,
            action_seq_len=action_horizon,
        )
        return actions_out, expert_latent
