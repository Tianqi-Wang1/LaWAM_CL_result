...
class LatentWorldPolicyConfig:
    ...
    # Routing-V1 auxiliary supervision for the Action-Expert latent branch.
    # z* is distilled to the frozen Stage-1 IDM latent z_GT, while the same z*
    # is decoded by LaWM and supervised directly against the real future DINO
    # feature h_GT.  The branch is auxiliary only; action inference is unchanged.
    enable_expert_latent_aux: bool = False
    expert_latent_distill_weight: float = 0.1
    expert_world_loss_weight: float = 0.1
    expert_latent_loss_type: str = "mse"  # "mse" | "cosine"
    ...
    # Routing-V2 task-specific path. Query deltas are zero-initialized residuals
    # over the Base latent/action queries; Base query parameters themselves stay
    # frozen during CL skill adaptation.
    routing_v2_enable_query_delta: bool = False

    # Routing-V2 task memories are trained in a separate second phase after the
    # policy skill path is frozen. They are not used by normal task-ID inference.
    routing_v2_enable_memory: bool = False
    routing_v2_memory_train_mode: bool = False
    routing_v2_semantic_bottleneck: int = 128
    routing_v2_dynamics_hidden: int = 192
    routing_v2_dynamics_layers: int = 2
    routing_v2_dynamics_heads: int = 6
    routing_v2_dynamics_ffn: int = 768
    routing_v2_dynamics_z_weight: float = 0.5
    routing_v2_dynamics_pred_weight: float = 0.5
    routing_v2_semantic_loss_weight: float = 1.0
    routing_v2_dynamics_loss_weight: float = 1.0
...
class LatentWorldPolicyBackend(nn.Module):
    ...
    def __init__(self, model_cfg: LatentWorldPolicyConfig, *, vlm_model_id: str) -> None:
        ...
        # Routing-V2 skill-specific query residuals.  Keeping these as separate
        # parameters preserves the exact Base query tensors in the checkpoint,
        # which are later reused as the common semantic-routing anchor.
        if bool(self.model_cfg.routing_v2_enable_query_delta):
            self.routing_v2_act_query_delta = nn.Parameter(torch.zeros_like(self.act_query))
            self.routing_v2_flow_query_delta = nn.Parameter(torch.zeros_like(self.flow_action_query))
        else:
            self.register_parameter("routing_v2_act_query_delta", None)
            self.register_parameter("routing_v2_flow_query_delta", None)
        ...
        if bool(self.model_cfg.flow_cfg.enable_expert_latent_head):
            configured_latent_dim = int(self.model_cfg.flow_cfg.expert_latent_dim)
            if configured_latent_dim != lam_code_dim:
                print(
                    "[LatentWorldPolicyBackend] auto-align expert latent head output dim: "
                    f"configured={configured_latent_dim}, lam.code_dim={lam_code_dim}."
                )
            self.model_cfg.flow_cfg.expert_latent_dim = lam_code_dim
        if bool(self.model_cfg.enable_expert_latent_aux) and not bool(
            self.model_cfg.flow_cfg.enable_expert_latent_head
        ):
            raise ValueError(
                "enable_expert_latent_aux=True requires "
                "flow_cfg.enable_expert_latent_head=True."
            )
        ...
        self.routing_v2_memory: Optional[RoutingV2Memory] = None
        if bool(self.model_cfg.routing_v2_enable_memory):
            self.routing_v2_memory = RoutingV2Memory(
                semantic_dim=vlm_hidden_dim,
                semantic_bottleneck=int(self.model_cfg.routing_v2_semantic_bottleneck),
                vision_dim=lam_vision_dim,
                latent_dim=lam_code_dim,
                num_tokens=lam_num_tokens,
                dynamics_hidden=int(self.model_cfg.routing_v2_dynamics_hidden),
                dynamics_layers=int(self.model_cfg.routing_v2_dynamics_layers),
                dynamics_heads=int(self.model_cfg.routing_v2_dynamics_heads),
                dynamics_ffn=int(self.model_cfg.routing_v2_dynamics_ffn),
                dynamics_z_weight=float(self.model_cfg.routing_v2_dynamics_z_weight),
            )
            print(
                "[RoutingV2][MEMORY] instantiated: "
                f"{self.routing_v2_memory.parameter_summary()}"
            )
        if bool(self.model_cfg.routing_v2_memory_train_mode) and self.routing_v2_memory is None:
            raise ValueError(
                "routing_v2_memory_train_mode=True requires routing_v2_enable_memory=True."
            )
    ...
    def _run_vlm_stage(
        ...
        *,
        ...
        pred_latent = self.vlm_to_lam(h_act)
        return {
            "h_vlm": hidden,
            "h_act": h_act,
    ...
    # Forward
    # ------------------
    def _run_routing_v2_base_anchor_hact(
        self,
        *,
        prepared_batch: LatentWorldPolicyTrainBatch,
    ) -> torch.Tensor:
        """Return Base-VLM + Base-query H_act even inside a task-specific skill.

        Only VLM LoRA is relevant to this feature path.  We temporarily disable
        its residual scaling and deliberately inject the immutable Base query
        parameters rather than Routing-V2 query deltas.
        """
        device = prepared_batch["input_ids"].device
        vlm_stage_dtype = self.model_cfg.vlm_dtype
        embed = self.vlm.get_input_embeddings()
        if embed is None:
            raise RuntimeError("Routing-V2 anchor requires VLM input embeddings.")
        vlm_embed_dtype = _module_param_dtype(embed, default=vlm_stage_dtype)
        base_act = self.act_query.to(device=device, dtype=vlm_embed_dtype)
        base_flow = self.flow_action_query.to(device=device, dtype=vlm_embed_dtype)
        with temporarily_disable_lora(self.vlm):
            with _cuda_autocast(vlm_stage_dtype):
                out = self._run_vlm_stage(
                    ...
                    act_query=base_act,
                    flow_query=base_flow,
                )
        return out["h_act"]

    def _forward_routing_v2_memory(
        self,
        *,
        prepared_batch: LatentWorldPolicyTrainBatch,
    ) -> Dict[str, torch.Tensor]:
        """Second-phase training for Semantic AE + Dynamics AE only."""
        if self.routing_v2_memory is None:
            raise RuntimeError("Routing-V2 memory forward requested without routing_v2_memory module.")
        device = prepared_batch["input_ids"].device

        # Policy / teacher features are immutable targets in this phase.
        with torch.no_grad():
            anchor_hact = self._run_routing_v2_base_anchor_hact(prepared_batch=prepared_batch).detach()
            skill = self._run_shared_encoding_train(
                prepared_batch=prepared_batch,
                source="LatentWorldPolicyBackend.routing_v2_memory",
                lam_features_with_no_grad=True,
            )
            teacher_z = self._run_lam_teacher(
                primary_video=prepared_batch["primary_video"],
                embodiment_id=prepared_batch["embodiment_id"],
            ).detach()
            h_t = skill.h_t.detach()
            h_gt = skill.h_t1_gt.detach()
            z_pred = skill.pred_action_emb.detach()
            h_pred = skill.h_t1_pred.detach()

        sem = self.routing_v2_memory.semantic(anchor_hact)
        dyn_gt = self.routing_v2_memory.dynamics(h_t=h_t, h_future=h_gt, z=teacher_z)
        dyn_pred = self.routing_v2_memory.dynamics(h_t=h_t, h_future=h_pred, z=z_pred)

        pred_w = float(self.model_cfg.routing_v2_dynamics_pred_weight)
        pred_w = max(0.0, min(1.0, pred_w))
        loss_dyn = (1.0 - pred_w) * dyn_gt["loss"] + pred_w * dyn_pred["loss"]
        loss_total = (
            float(self.model_cfg.routing_v2_semantic_loss_weight) * sem["loss"]
            + float(self.model_cfg.routing_v2_dynamics_loss_weight) * loss_dyn
        )
        zero = torch.zeros((), device=device, dtype=loss_total.dtype)
        return {
            "loss_flow": zero,
            ...
            "loss_routing_v2_semantic": sem["loss"],
            "loss_routing_v2_dynamics": loss_dyn,
            "loss_routing_v2_dynamics_gt": dyn_gt["loss"],
            "loss_routing_v2_dynamics_pred": dyn_pred["loss"],
            "diag_routing_v2_dyn_gt_delta": dyn_gt["loss_delta"],
            "diag_routing_v2_dyn_gt_z": dyn_gt["loss_z"],
            "diag_routing_v2_dyn_pred_delta": dyn_pred["loss_delta"],
            "diag_routing_v2_dyn_pred_z": dyn_pred["loss_z"],
        }
    ...
    def _prepare_queries(self, *, device: torch.device, vlm_stage_dtype: torch.dtype) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        act_delta = getattr(self, "routing_v2_act_query_delta", None)
        flow_delta = getattr(self, "routing_v2_flow_query_delta", None)
        if act_delta is not None:
            act_query = act_query + act_delta
        if flow_delta is not None:
            flow_query = flow_query + flow_delta
        act_query = act_query.to(device=device, dtype=vlm_embed_dtype)
    ...
    def forward(
        ...
        *,
        ...
        if bool(self.model_cfg.routing_v2_memory_train_mode):
            return self._forward_routing_v2_memory(prepared_batch=prepared_batch)
        ...
        enable_expert_aux = bool(self.model_cfg.enable_expert_latent_aux)
        need_teacher_latent = bool(self.model_cfg.enable_loss_distill) or enable_expert_aux
        teacher_latent: Optional[torch.Tensor] = None

        with _cuda_autocast(lam_stage_dtype):
            if need_teacher_latent:
                # Stage-1 IDM teacher; always frozen/no-grad inside _run_lam_teacher.
                teacher_latent = self._run_lam_teacher(
                    primary_video=prepared_batch["primary_video"],
                    embodiment_id=prepared_batch["embodiment_id"],
                )
            ...
            if bool(self.model_cfg.enable_loss_distill):
                ...
                loss_distill = self._compute_latent_loss(
        ...
        expert_latent: Optional[torch.Tensor] = None
        with _cuda_autocast(flow_stage_dtype):
            flow_out = self.flow(
                ...
                return_expert_latent=enable_expert_aux,
            )
            if enable_expert_aux:
                if not isinstance(flow_out, tuple) or len(flow_out) != 2:
                    raise RuntimeError(
                        "Flow head must return (loss_flow, z_star) when expert latent auxiliary training is enabled."
                    )
                loss_flow, expert_latent = flow_out
            else:
                if isinstance(flow_out, tuple):
                    loss_flow = flow_out[0]
                else:
                    loss_flow = flow_out

        zero = torch.tensor(0.0, device=device, dtype=loss_flow.dtype)
        loss_expert_latent = zero
        loss_expert_world = zero
        diag_zstar_to_zhat = zero
        diag_zhat_to_zgt = zero
        diag_hstar_to_hhat = zero
        expert_future: Optional[torch.Tensor] = None

        if enable_expert_aux:
            assert expert_latent is not None
            assert teacher_latent is not None
            teacher_latent_rep = teacher_latent.repeat(
                repeat_steps, *([1] * (teacher_latent.ndim - 1))
            )
            online_latent_rep = shared.pred_action_emb.repeat(
                repeat_steps, *([1] * (shared.pred_action_emb.ndim - 1))
            )

            with _cuda_autocast(lam_stage_dtype):
                loss_expert_latent = self._compute_latent_loss(
                    pred_latent=expert_latent,
                    teacher_latent=teacher_latent_rep,
                    latent_loss_type=self.model_cfg.expert_latent_loss_type,
                )
                # Keep LaWM differentiable with respect to z*.  During CL the
                # freeze policy freezes LaWM parameters; during Base training
                # the decoder remains trainable, exactly as requested.
                expert_future = self._decode_future_tokens_strict_single_query(
                    h_t=h_t_rep,
                    pred_action_emb=expert_latent,
                    source="LatentWorldPolicyBackend.forward/expert_latent",
                )
                loss_expert_world = F.mse_loss(expert_future, h_t1_gt_rep)

            # Diagnostics only.  These do not affect optimization and let us
            # compare GT-anchored supervision with the actual inference-time
            # online spaces (z_hat and h_hat) before designing routing.
            with torch.no_grad():
                diag_zstar_to_zhat = F.mse_loss(
                    expert_latent.detach().float(), online_latent_rep.detach().float()
                )
                diag_zhat_to_zgt = F.mse_loss(
                    online_latent_rep.detach().float(), teacher_latent_rep.detach().float()
                )
                assert expert_future is not None
                diag_hstar_to_hhat = F.mse_loss(
                    expert_future.detach().float(), h_t1_pred_rep.detach().float()
                )
        ...
        with _cuda_autocast(flow_stage_dtype):
            loss_total = (
                ...
                + self.model_cfg.expert_latent_distill_weight * loss_expert_latent
                + self.model_cfg.expert_world_loss_weight * loss_expert_world
            )
        ...
        # Keep the legacy output contract exact when Routing-V1 is disabled.
        if enable_expert_aux:
            output.update(
                {
                    "loss_expert_latent": loss_expert_latent,
                    "loss_expert_world": loss_expert_world,
                    "diag_zstar_to_zhat": diag_zstar_to_zhat,
                    "diag_zhat_to_zgt": diag_zhat_to_zgt,
                    "diag_hstar_to_hhat": diag_hstar_to_hhat,
                }
            )
        return output
