...
class TrainerUtils:
    ...
    def load_finetune_init_weights(model, checkpoint_path: str, load_pretrained_policy_flow: bool = True):
        ...
        for key, value in checkpoint.items():
            is_policy_flow_key = (
                ...
                or key == "policy_action_head"
                or key.startswith("policy_action_head.")
            )
            if (not load_pretrained_policy_flow) and is_policy_flow_key:
        ...
        if skipped_policy_flow_keys:
            ...
            _info(
                "Relaxed finetune init skipped Flow aliases (`policy_backend.flow` + `policy_action_head`) for `%s`: count=%d sample=%s",
                checkpoint_path,
        ...
        if intentionally_skipped_keys:
            _info(
                "Relaxed finetune init kept Flow aliases at existing initialization for `%s`: count=%d sample=%s",
                checkpoint_path,
    ...
    @staticmethod
    def load_policy_flow_override_weights(model, checkpoint_path: str):
        """Override only the pre-existing Flow core from another full checkpoint.

        This is used for controlled experiments such as:
          Base upstream + PRETRAIN Flow core + fresh residual expert.

        Residual-expert tensors are intentionally not loaded even if the source
        checkpoint contains them. Canonical `policy_backend.flow.*` keys are
        preferred; the legacy `policy_action_head.*` alias is accepted as fallback.
        """
        if not checkpoint_path:
            return model
        if (not dist.is_initialized()) or dist.get_rank() == 0:
            print(f"📦 policy Flow override checkpoint: {checkpoint_path}")

        try:
            checkpoint = torch.load(checkpoint_path, map_location="cpu")
        except Exception as e:
            raise RuntimeError(f"❌ loading Flow override checkpoint failed: {e}") from e
        if not isinstance(checkpoint, dict):
            raise RuntimeError(
                f"❌ expected Flow override checkpoint to be a dict, got {type(checkpoint).__name__}"
            )

        target_state = model.state_dict()
        canonical_prefix = "policy_backend.flow."
        alias_prefix = "policy_action_head."
        residual_marker = ".residual_expert_blocks."

        # Detect inconsistent duplicate aliases in the source before loading.
        source_suffixes = set()
        for key in checkpoint:
            if key.startswith(canonical_prefix):
                source_suffixes.add(key[len(canonical_prefix):])
            elif key.startswith(alias_prefix):
                source_suffixes.add(key[len(alias_prefix):])
        for suffix in sorted(source_suffixes):
            if residual_marker.strip(".") in suffix:
                continue
            ck = canonical_prefix + suffix
            ak = alias_prefix + suffix
            if ck in checkpoint and ak in checkpoint:
                a, b = checkpoint[ck], checkpoint[ak]
                if torch.is_tensor(a) and torch.is_tensor(b):
                    if tuple(a.shape) != tuple(b.shape) or a.dtype != b.dtype or not torch.equal(a, b):
                        raise RuntimeError(
                            "Flow override source has inconsistent canonical/alias tensors: "
                            f"{ck} vs {ak}"
                        )
        ...
        for target_key, target_value in target_state.items():
            if not target_key.startswith(canonical_prefix):
                continue
            suffix = target_key[len(canonical_prefix):]
            if "residual_expert_blocks." in suffix:
                continue
            src_key = None
            for candidate in (canonical_prefix + suffix, alias_prefix + suffix):
            ...
            if src_key is None:
                ...
                continue
            src_value = checkpoint[src_key]
            if hasattr(src_value, "shape") and tuple(src_value.shape) != tuple(target_value.shape):
                shape_mismatch.append((target_key, tuple(src_value.shape), tuple(target_value.shape)))
                continue
            override[target_key] = src_value

        if missing or shape_mismatch:
            raise RuntimeError(
                "Flow override is incomplete/incompatible: "
                f"missing={missing[:20]}, shape_mismatch={shape_mismatch[:20]}"
            )
        if not override:
            raise RuntimeError("Flow override found no compatible pre-existing Flow tensors.")

        load_result = model.load_state_dict(override, strict=False)
        # Missing keys are expected because we intentionally load only canonical Flow core.
        if load_result.unexpected_keys:
            raise RuntimeError(
                f"Unexpected keys while loading Flow override: {load_result.unexpected_keys[:20]}"
            )

        if (not dist.is_initialized()) or dist.get_rank() == 0:
            print(
                "✅ Flow core override loaded: "
                f"{len(override)} canonical tensors; residual experts kept at fresh initialization."
            )
        return model
