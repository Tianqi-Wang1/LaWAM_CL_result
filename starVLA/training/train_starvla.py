...
TRAIN_COMPONENT_METRIC_ALIASES = {
    ...
    "train_loss_expert_latent": ("loss_expert_latent",),
    "train_loss_expert_world": ("loss_expert_world",),
    "train_diag_zstar_to_zhat": ("diag_zstar_to_zhat",),
    "train_diag_zhat_to_zgt": ("diag_zhat_to_zgt",),
    "train_diag_hstar_to_hhat": ("diag_hstar_to_hhat",),
    "train_loss_routing_v2_semantic": ("loss_routing_v2_semantic",),
    "train_loss_routing_v2_dynamics": ("loss_routing_v2_dynamics",),
    "train_loss_routing_v2_dynamics_gt": ("loss_routing_v2_dynamics_gt",),
    "train_loss_routing_v2_dynamics_pred": ("loss_routing_v2_dynamics_pred",),
    "train_diag_routing_v2_dyn_gt_delta": ("diag_routing_v2_dyn_gt_delta",),
    "train_diag_routing_v2_dyn_gt_z": ("diag_routing_v2_dyn_gt_z",),
    "train_diag_routing_v2_dyn_pred_delta": ("diag_routing_v2_dyn_pred_delta",),
    "train_diag_routing_v2_dyn_pred_z": ("diag_routing_v2_dyn_pred_z",),
}
...
class VLATrainer(TrainerUtils):
    ...
    def _init_checkpointing(self):
        ...
        flow_override_checkpoint = getattr(
            self.config.trainer, "policy_flow_override_checkpoint", None
        )
        ...
        if pretrained_checkpoint:
            ...
            if flow_override_checkpoint:
                self.model = self.load_policy_flow_override_weights(
                    self.model,
                    checkpoint_path=flow_override_checkpoint,
                )
            self.completed_steps = 0
            logger.info(
                "Initialized model weights for finetune from `%s`; load_pretrained_policy_flow=%s; "
                "policy_flow_override_checkpoint=%s; training starts from step 0.",
                pretrained_checkpoint,
                load_pretrained_policy_flow,
                flow_override_checkpoint,
            )
        else:
            if flow_override_checkpoint:
                raise ValueError(
                    "trainer.policy_flow_override_checkpoint requires trainer.pretrained_checkpoint "
                    "so the non-Flow model has a defined initialization."
                )
    ...
    def eval_action_model(self, step_metrics: dict = None) -> float:
        ...
        val_loss_expert_latent_numerator = torch.zeros((), dtype=torch.float64, device=self.accelerator.device)
        val_loss_expert_latent_denominator = torch.zeros((), dtype=torch.float64, device=self.accelerator.device)
        val_loss_expert_world_numerator = torch.zeros((), dtype=torch.float64, device=self.accelerator.device)
        val_loss_expert_world_denominator = torch.zeros((), dtype=torch.float64, device=self.accelerator.device)
        val_diag_zstar_to_zhat_numerator = torch.zeros((), dtype=torch.float64, device=self.accelerator.device)
        val_diag_zstar_to_zhat_denominator = torch.zeros((), dtype=torch.float64, device=self.accelerator.device)
        val_diag_zhat_to_zgt_numerator = torch.zeros((), dtype=torch.float64, device=self.accelerator.device)
        val_diag_zhat_to_zgt_denominator = torch.zeros((), dtype=torch.float64, device=self.accelerator.device)
        val_diag_hstar_to_hhat_numerator = torch.zeros((), dtype=torch.float64, device=self.accelerator.device)
        val_diag_hstar_to_hhat_denominator = torch.zeros((), dtype=torch.float64, device=self.accelerator.device)
            ...
            with torch.no_grad():
                for _ in range(eval_batches):
                    ...
                    try:
                        ...
                        _accumulate_eval_scalar(
                            val_loss_expert_latent_numerator,
                            val_loss_expert_latent_denominator,
                            eval_output_dict.get("loss_expert_latent"),
                        )
                        _accumulate_eval_scalar(
                            val_loss_expert_world_numerator,
                            val_loss_expert_world_denominator,
                            eval_output_dict.get("loss_expert_world"),
                        )
                        _accumulate_eval_scalar(
                            val_diag_zstar_to_zhat_numerator,
                            val_diag_zstar_to_zhat_denominator,
                            eval_output_dict.get("diag_zstar_to_zhat"),
                        )
                        _accumulate_eval_scalar(
                            val_diag_zhat_to_zgt_numerator,
                            val_diag_zhat_to_zgt_denominator,
                            eval_output_dict.get("diag_zhat_to_zgt"),
                        )
                        _accumulate_eval_scalar(
                            val_diag_hstar_to_hhat_numerator,
                            val_diag_hstar_to_hhat_denominator,
                            eval_output_dict.get("diag_hstar_to_hhat"),
                        )
        ...
        if dist.is_initialized():
            ...
            dist.all_reduce(val_loss_expert_latent_numerator, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_loss_expert_latent_denominator, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_loss_expert_world_numerator, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_loss_expert_world_denominator, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_diag_zstar_to_zhat_numerator, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_diag_zstar_to_zhat_denominator, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_diag_zhat_to_zgt_numerator, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_diag_zhat_to_zgt_denominator, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_diag_hstar_to_hhat_numerator, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_diag_hstar_to_hhat_denominator, op=dist.ReduceOp.SUM)
        ...
        if self.accelerator.is_main_process:
            ...
            optional_val_metrics = (
                ("val_loss_expert_latent", val_loss_expert_latent_numerator, val_loss_expert_latent_denominator),
                ("val_loss_expert_world", val_loss_expert_world_numerator, val_loss_expert_world_denominator),
                ("val_diag_zstar_to_zhat", val_diag_zstar_to_zhat_numerator, val_diag_zstar_to_zhat_denominator),
                ("val_diag_zhat_to_zgt", val_diag_zhat_to_zgt_numerator, val_diag_zhat_to_zgt_denominator),
                ("val_diag_hstar_to_hhat", val_diag_hstar_to_hhat_numerator, val_diag_hstar_to_hhat_denominator),
            )
            for key, numerator, denominator in optional_val_metrics:
                if float(denominator.item()) > 0:
                    step_metrics[key] = float((numerator / denominator).item())
