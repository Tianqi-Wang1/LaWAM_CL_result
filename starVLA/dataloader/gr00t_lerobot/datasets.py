...
def calculate_dataset_statistics(
    ...
    """
    Calculate dataset statistics from parquet files.

    If `task_indices` is provided, only rows whose task_index is
    included in `task_indices` are used. This is used by the
    continual-learning Base stage to prevent future-task
    information from entering normalization statistics.
    """
    ...
    if task_indices is not None:
        ...
        if len(selected_task_indices) == 0:
            raise ValueError(
                "`task_indices` for statistics cannot be empty."
            )

        print(
            "[LeRobotDataset][statistics] "
            f"Filtering statistics to task_indices="
    ...
    for parquet_path in tqdm(
        ...
        total_rows_before_filter += len(
            parquet_data
        )

        # --------------------------------------------------
        # Optional CL Base filtering
        # --------------------------------------------------
        if selected_task_indices is not None:

            if "task_index" not in parquet_data.columns:
                raise KeyError(
                    "`task_index` is required for "
                    "task-filtered statistics, but it is "
                    f"missing from {parquet_path}."
                )

            parquet_data = parquet_data[
                parquet_data["task_index"].isin(
                    selected_task_indices
                )
            ]
        ...
        if len(parquet_data) > 0:
            all_low_dim_data_list.append(
                parquet_data
            )

    if not all_low_dim_data_list:
        raise ValueError(
            "No rows remain while calculating dataset "
            f"statistics. task_indices="
            f"{selected_task_indices}"
        )

    all_low_dim_data = pd.concat(
        all_low_dim_data_list,
        axis=0,
        ignore_index=True,
    )
    ...
    if selected_task_indices is not None:
        print(
            ...
            f"{total_rows_before_filter} "
            f"rows_after_filter="
            f"{total_rows_after_filter}"
        )

        actual_task_indices = sorted(
            int(x)
            for x in
            all_low_dim_data[
                "task_index"
        ...
        print(
            "[LeRobotDataset][statistics] "
            f"actual_task_indices="
            f"{actual_task_indices}"
        )

        missing = (
            selected_task_indices
            - set(actual_task_indices)
        )

        if missing:
            raise ValueError(
                "Some requested task indices contain no "
                f"rows: {sorted(missing)}"
            )

    # ------------------------------------------------------
    # Original statistics computation
    # ------------------------------------------------------
    dataset_statistics = {}

    for le_modality in tqdm(
        all_low_dim_data.columns,
        ...
        try:
            ...
            print(
                "Warning: Failed to process modality "
                f"{le_modality} due to error: {e}"
            )
            continue
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ----------------------------------------------------------
        # Optional task-level filtering for continual learning.
        #
        # `task_indices` refers to task indices in the merged
        # LeRobot dataset (i.e. meta/tasks.parquet), not the
        # suite-local LIBERO task IDs.
        #
        # None -> preserve original behavior and use all tasks.
        # ----------------------------------------------------------
        self._selected_task_indices = self._resolve_task_indices_filter()

        # self._episodes = self._get_episode_info()
        self._tasks = self._get_tasks()
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        #
        # CL Base:
        #   meta/stats_gr00t_tasks_<indices>.json
        # ----------------------------------------------------------
        ...
        if self.data_cfg is not None:
            if hasattr(self.data_cfg, "get"):
                use_task_filtered_statistics = bool(
                    self.data_cfg.get(
                        "use_task_filtered_statistics",
                        False,
                    )
                )
            else:
        ...
        statistics_task_indices = None

        if use_task_filtered_statistics:
            ...
            if statistics_task_indices is None:
                raise ValueError(
                    "`use_task_filtered_statistics=true` "
                    "requires `task_indices`. "
                    "For CL experiments this should normally "
                    "come from `cl_suite + cl_task_ids`."
                )

            statistics_task_indices = sorted(
                {
                    int(x)
                    for x in statistics_task_indices
                }
            )

            if len(statistics_task_indices) == 0:
                raise ValueError(
                    "Statistics task filter is empty."
                )
            ...
            stats_path = (
                self.dataset_path
                / "meta"
                / f"stats_gr00t_tasks_{task_suffix}.json"
            )

            print(
                "[LeRobotDataset][statistics] "
                "Using task-filtered normalization "
                f"statistics: task_indices="
                f"{statistics_task_indices}"
            )

            print(
                "[LeRobotDataset][statistics] "
                f"cache_path={stats_path}"
            )
        ...
        tmp_path = stats_path.with_suffix(
            stats_path.suffix + ".tmp"
        )
        ...
        if le_statistics is None and is_main():
            ...
            le_statistics = calculate_dataset_statistics(
                parquet_files_filtered,
                task_indices=statistics_task_indices,
            )
    ...
    def _get_trajectories(
        ...
        """
        Get trajectories in the dataset, optionally filtered by task.
        """
        if (
            not hasattr(self, "episodes_hf")
            or self.episodes_hf is None
        ):
            raise RuntimeError(
                "LeRobot v3.0 requires _load_episodes_hf() "
                "to be called before _get_trajectories()"
            )

        trajectory_ids = []
        trajectory_lengths = []

        # Always keep the complete episode_index -> metadata-row
        # mapping, since later frame lookup relies on it.
        self._episode_index_to_row: dict[int, int] = {}

        # This mapping contains only trajectories active before
        # train/val splitting (i.e. after optional task filtering).
        self._trajectory_id_to_index: dict[int, int] = {}

        # Debug / verification statistics for CL filtering.
        selected_episode_counts: dict[int, int] = defaultdict(int)
        selected_frame_counts: dict[int, int] = defaultdict(int)
        ...
        for i in range(len(self.episodes_hf)):
            ...
            # Keep metadata lookup for every episode.
            self._episode_index_to_row[ep_idx] = i

            task_index = self._get_episode_task_index(row)

            # --------------------------------------------------
            # CL task filtering
            # --------------------------------------------------
            if (
                self._selected_task_indices is not None
                and task_index
                not in self._selected_task_indices
            ):
                continue
            ...
            trajectory_lengths.append(ep_length)

            selected_episode_counts[task_index] += 1
            selected_frame_counts[task_index] += ep_length

        print(
            "[INFO] Built episode_index->row mapping "
            f"({len(self._episode_index_to_row)} total episodes)"
        )

        if self._selected_task_indices is not None:
            print(
                "[LeRobotDataset][task_filter] "
                f"kept_episodes={len(trajectory_ids)} "
                f"kept_frames={sum(trajectory_lengths)}"
            )
            ...
            for task_index in sorted(
                ...
                print(
                    ...
                    f"episodes="
                    f"{selected_episode_counts[task_index]} "
                    f"frames="
                    f"{selected_frame_counts[task_index]}"
                )

        if len(trajectory_ids) == 0:
            raise ValueError(
                "Task filtering removed all trajectories. "
                f"Selected task indices: "
                f"{self._selected_task_indices}"
            )

        # Detect video keys from episode columns.
        self._detected_video_keys = []

        for col in self.episodes_hf.column_names:
            if (
                col.startswith("videos/")
                and col.endswith("/from_timestamp")
            ):
                vid_key = (
                    col
                    .replace("videos/", "")
                    .replace("/from_timestamp", "")
                )

                self._detected_video_keys.append(
                    vid_key
                )
    ...
    def _resolve_task_indices_filter(self) -> set[int] | None:
        """
        Resolve an optional task filter from data_cfg.

        The configured values correspond to task_index in the merged
        LeRobot dataset. If no filter is provided, return None so the
        original all-task behavior is preserved.
        """
        if self.data_cfg is None:
            return None

        if hasattr(self.data_cfg, "get"):
            raw_task_indices = self.data_cfg.get("task_indices", None)
        else:
            raw_task_indices = getattr(
                self.data_cfg,
                "task_indices",
                None,
            )

        if raw_task_indices is None:
            return None

        # OmegaConf ListConfig, list, tuple, etc.
        if isinstance(raw_task_indices, (int, np.integer)):
            raw_task_indices = [int(raw_task_indices)]
        ...
        if len(task_indices) == 0:
            raise ValueError(
                "`task_indices` was provided but is empty. "
                "Use null to disable task filtering."
            )

        available_task_indices = set(
            int(x)
            for x in self._tasks["task_index"].tolist()
        )

        invalid = sorted(
            task_indices - available_task_indices
        )

        if invalid:
            raise ValueError(
                "Requested task indices are not present in "
                f"{self.dataset_name}: {invalid}. "
                f"Available task indices: "
                f"{sorted(available_task_indices)}"
            )

        selected_names = {
            idx: self._task_index_to_task.get(idx, "")
        ...
        print(
            ...
            f"selected_task_indices={sorted(task_indices)}"
        )

        for idx, task_name in selected_names.items():
            print(
                "[LeRobotDataset][task_filter] "
                f"task_index={idx} task={task_name}"
            )

        return task_indices
    
    def _get_episode_task_index(
        self,
        episode_row: dict,
    ) -> int:
        """
        Resolve the task_index for one episode.

        Prefer episode-level metadata when available. Otherwise,
        recover it from the first frame of the episode in hf_dataset.
        """

        # Some LeRobot datasets may already store task_index
        # directly in episode metadata.
        if "task_index" in episode_row:
            value = episode_row["task_index"]

            if isinstance(value, (list, tuple, np.ndarray)):
                if len(value) == 0:
                    raise ValueError(
                        "Empty task_index in episode metadata."
                    )
                value = value[0]

            return int(value)

        # LIBERO merged export: infer it from the first frame.
        from_index = int(
            episode_row["dataset_from_index"]
        )

        first_frame = self.hf_dataset[from_index]

        if "task_index" not in first_frame:
            raise KeyError(
                "`task_index` is missing from both episode metadata "
                "and frame-level dataset."
            )
