...
from starVLA.dataloader.libero_task_registry import (
    resolve_libero_task_indices,
)
...
def get_vla_dataset(
    ...
    **kwargs: dict,
    ...
    For continual-learning experiments, the preferred interface is:

        cl_suite: libero_goal
        cl_task_ids: [0, 1, 2, ...]

    These suite-local LIBERO task IDs are converted here into the
    `task_index` values used by the merged LeRobot dataset.

    The lower-level `task_indices` option is still supported for
    debugging, but it should normally remain null.
    """
    ...
    #
    # Important:
    # build_dataloaders() constructs train and validation datasets
    # separately. We therefore should NOT permanently modify the
    # original OmegaConf object passed from the training config.
    # ----------------------------------------------------------
    resolved_data_cfg = copy.deepcopy(data_cfg)

    # ----------------------------------------------------------
    # Resolve high-level LIBERO CL task IDs.
    # ----------------------------------------------------------
    cl_suite = _cfg_get(
        resolved_data_cfg,
        "cl_suite",
    ...
    cl_task_ids = _cfg_get(
        resolved_data_cfg,
        "cl_task_ids",
        None,
    )

    direct_task_indices = _cfg_get(
        resolved_data_cfg,
        "task_indices",
        None,
    )

    # cl_suite and cl_task_ids must appear together.
    if (cl_suite is None) != (cl_task_ids is None):
        raise ValueError(
            "`cl_suite` and `cl_task_ids` must either both "
            "be provided or both be null."
        )

    # Preferred CL interface.
    if cl_suite is not None:

        # Avoid ambiguous configurations.
        if direct_task_indices is not None:
            raise ValueError(
                "Do not specify both `cl_task_ids` and "
                "`task_indices`. "
                "For normal CL experiments, use "
                "`cl_suite + cl_task_ids`."
            )

        resolved_task_indices = (
            resolve_libero_task_indices(
                suite=str(cl_suite),
                task_ids=cl_task_ids,
            )
        )

        print(
            "[LeRobotDataset][cl_mapping] "
            f"suite={cl_suite} "
            f"local_task_ids="
            f"{[int(x) for x in cl_task_ids]} "
            f"-> merged_task_indices="
            f"{resolved_task_indices}"
        )

        # This is the field already consumed by the low-level
        # task filtering logic in LeRobotSingleDataset.
        resolved_data_cfg.task_indices = (
            resolved_task_indices
        )

    # ----------------------------------------------------------
    # Everything below is essentially the original LaWAM logic,
    # but uses `resolved_data_cfg` instead of `data_cfg`.
    # ----------------------------------------------------------

    data_root_dir = resolved_data_cfg.data_root_dir
    data_mix = resolved_data_cfg.data_mix

    mixture_spec = DATASET_NAMED_MIXTURES[
        data_mix
    ]
    ...
    for d_name, d_weight, robot_type in mixture_spec:
        ...
        if dataset_key in included_datasets:
            print(
                ...
                f"`{(d_name, d_weight, robot_type)}`"
            )
            continue

        included_datasets.add(
            dataset_key
        )

        filtered_mixture_spec.append(
            (
                d_name,
                d_weight,
                robot_type,
