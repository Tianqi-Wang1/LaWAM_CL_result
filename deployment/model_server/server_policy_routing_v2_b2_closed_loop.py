...
class RoutingV2B2ClosedLoopPolicy:
    ...
    def __init__(
        ...
        *,
        ...
        if not self.tasks:
            raise ValueError("Routing-V2 B2 requires at least one candidate skill")
    ...
    def predict_action(self, examples: Sequence[dict], return_intermediates: bool = False, **kwargs):
        if not examples:
            raise ValueError("Routing-V2 B2 closed-loop predict_action requires non-empty examples")
        if kwargs:
        ...
        if any(x is None for x in out_actions):
            raise RuntimeError("Routing-V2 B2 failed to produce actions for every sample")
...
def main(args) -> None:
    ...
    wrapper = RoutingV2B2ClosedLoopPolicy(
        policy=policy,
    ...
    metadata = build_policy_server_metadata(
        ...
        server_type="routing_v2_b2_closed_loop",
        env="generic",
        supported_eval_envs=["libero"],
        extra_metadata={
            "routing_v2_b2_closed_loop": True,
    ...
    logging.info("Creating Routing-V2 B2 closed-loop server host=%s ip=%s", hostname, local_ip)
