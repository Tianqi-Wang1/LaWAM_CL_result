...
FLOW_LORA_RANK="${FLOW_LORA_RANK:-8}"
FLOW_LORA_ALPHA="${FLOW_LORA_ALPHA:-8.0}"
FLOW_LORA_DROPOUT="${FLOW_LORA_DROPOUT:-0.0}"

EXPERIMENT_ROOT="/home/jincai_guo/tianqi/CVPR2027/checkpoints/lawam_cl/libero_goal/t9_flow_interface_followup_v4/ft_last8_interface_lora"
RUN_ROOT="${EXPERIMENT_ROOT}/runs"; LOG_ROOT="${EXPERIMENT_ROOT}/logs"
OUTPUT_ROOT="${ROOT}/results/eval_runs/lawam_cl/libero_goal/t9_flow_interface_followup_v4/ft_last8_interface_lora"
...
RUN_ID="t9_ft_last8_interface_lora_${MAX_TRAIN_STEPS}step_4gpu_bs${PER_DEVICE_BATCH_SIZE}_ga${GRADIENT_ACCUMULATION_STEPS}"
MASTER_LOG="${LOG_ROOT}/${RUN_ID}_${STAMP}.log"
exec > >(tee -a "${MASTER_LOG}") 2>&1

echo "=========================================================="
echo " T9 FT-LAST8 + FLOW-INTERFACE LoRA"
echo "=========================================================="
echo "Base          : ${BASE_CKPT}"
...
export MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-30033}"
...
bash train_lawam.sh \
    ...
    "--trainer.freeze.train_flow_interface_lora=true" \
    "--trainer.freeze.flow_lora_rank=${FLOW_LORA_RANK}" \
    "--trainer.freeze.flow_lora_alpha=${FLOW_LORA_ALPHA}" \
    "--trainer.freeze.flow_lora_dropout=${FLOW_LORA_DROPOUT}" \
    "--trainer.freeze.flow_interface_target_enc_vlm=true" \
    "--trainer.freeze.flow_interface_target_action_encoder=true" \
    "--trainer.freeze.flow_interface_target_action_decoder=true" \
    "--trainer.freeze.flow_interface_target_state_encoder=true" \
    "--trainer.freeze.flow_interface_target_output=true" \
    "--trainer.freeze.flow_interface_target_timestep=true" \
...
verify_run "T9 FT-LAST8 + FLOW-INTERFACE LoRA" "${RUN}"
...
if bool(fr.train_flow_interface_dense) != False: raise RuntimeError('Wrong interface-dense flag')
if bool(fr.train_flow_interface_lora) != True: raise RuntimeError('Wrong interface-LoRA flag')
...
python "${VERIFY_SCRIPT}" --base-ckpt "${BASE_CKPT}" --final-ckpt "${RUN}/final_model/pytorch_model.pt" \
  --layers "${DENSE_LAYER_CSV}" --interface-mode lora

MERGED_CKPT="${RUN}/final_model/pytorch_model_interface_merged.pt"
ADAPTER_CKPT="${RUN}/final_model/flow_interface_lora_adapter.pt"
python "${MERGE_SCRIPT}" "${RUN}/final_model/pytorch_model.pt" \
  --output "${MERGED_CKPT}" --adapter-output "${ADAPTER_CKPT}" --alpha "${FLOW_LORA_ALPHA}"
python "${VERIFY_SCRIPT}" --base-ckpt "${BASE_CKPT}" --final-ckpt "${MERGED_CKPT}" \
  --layers "${DENSE_LAYER_CSV}" --interface-mode merged_lora
...
ALIAS="t9_ft_last8_interface_lora"
...
bash examples/LIBERO/eval_files/auto_eval_scripts/run_libero_benchmark.sh "${MERGED_CKPT}"
...
echo "=========================================================="
echo " T9 FT-LAST8 + FLOW-INTERFACE LoRA COMPLETE"
echo " Run       : ${RUN}"
