...
# =============================================================================
# C0: LIBERO-Goal T9 protocol control
#     Formal Base checkpoint -> freeze all upstream -> FULL fine-tune Base Flow
#
# Parallel-friendly features:
#   - TARGET_GLOBAL_BATCH_SIZE defaults to 256 and gradient accumulation is
#     auto-computed when GRADIENT_ACCUMULATION_STEPS is not explicitly set.
#   - Dedicated default MAIN_PROCESS_PORT=29531.
#   - DO_EVAL=false can be used for parallel training-only runs.
#   - DO_TRAIN=false + RUN_DIR_OVERRIDE=/path/to/run can be used for eval-only.
# =============================================================================
...
EXPERIMENT_ROOT="/home/jincai_guo/tianqi/CVPR2027/checkpoints/lawam_cl/libero_goal/t9_residual_expert_split_v1/full_flow_from_base"
RUN_ROOT="${EXPERIMENT_ROOT}/runs"
LOG_ROOT="${EXPERIMENT_ROOT}/logs"
OUTPUT_ROOT="${ROOT}/results/eval_runs/lawam_cl/libero_goal/t9_residual_expert_split_v1/full_flow_from_base"
...
if [ -z "${GRADIENT_ACCUMULATION_STEPS+x}" ]; then
    ...
    if [ $((TARGET_GLOBAL_BATCH_SIZE % DENOM)) -ne 0 ]; then
        ...
        echo "        Set GRADIENT_ACCUMULATION_STEPS explicitly or adjust batch size."
        exit 1
    fi
...
MASTER_LOG="${LOG_ROOT}/full_flow_from_base_${PIPELINE_TIMESTAMP}.log"
exec > >(tee -a "${MASTER_LOG}") 2>&1

RUN_ID="t9_full_flow_from_base_${MAX_TRAIN_STEPS}step_gbs${GLOBAL_BATCH_SIZE}"

echo "=========================================================="
echo " C0: Base -> Full Flow FT -> T9"
echo "=========================================================="
...
verify_run "C0 full-flow from Base" "${RUN}"
...
if [ "${DO_EVAL}" = "true" ]; then
    ...
    LIBERO_CKPT_ALIAS="full_flow_from_base" \
    bash examples/LIBERO/eval_files/auto_eval_scripts/run_libero_benchmark.sh "${RUN}/final_model/pytorch_model.pt"

    EVAL_DIR=$(find "${EVAL_MASTER}/full_flow_from_base" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)
