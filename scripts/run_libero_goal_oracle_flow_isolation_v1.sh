...
# Purpose
# -------
# Diagnostic experiment to isolate temporal forgetting of the Flow action head.
# This is NOT a final task-ID-free continual-learning strategy.
#
# We keep one historical Flow head for each introduction stage:
#
...
#   F_CL4  : task 9
#
# Shared upstream modules continue learning sequentially.
#
# Training rule
# -------------
...
#
# Therefore F_CL1, F_CL2, F_CL3, F_CL4 are independently adapted from F_Base,
# rather than inheriting the previously adapted CL Flow head.
#
# Evaluation rule
# ---------------
# At stage CLk, use the CURRENT shared state but the Flow head from the task's
# introduction stage:
#
...
#
# The correct historical head is selected with oracle task identity. This removes
# Flow-head temporal overwrite from the behavioral diagnostic as much as possible.
#
# IMPORTANT COMPONENT BOUNDARY
# ----------------------------
# Only `policy_backend.flow.*` is treated as the Flow head, matching the existing
# LaWAM action-coupling analysis. `policy_backend.flow_action_query` remains in
# the shared continually learned state.
#
# Variants
# --------
# A) seqft_oracle_flow
#    Original formal SeqFT VLM policy:
#      vision backbone trainable
#      LLM backbone trainable
#      vision merger trainable
#      embeddings frozen
#
# B) freeze_backbone_oracle_flow
#    Structured VLM policy:
#      vision backbone frozen
#      LLM backbone frozen
#      vision merger trainable
#      embeddings frozen
#
# Both keep QFormer / LaWM / Flow training otherwise identical.
#
# Default:
#   bash scripts/run_libero_goal_oracle_flow_isolation_v1.sh
#
# Run only SeqFT-OracleFlow:
#   SEQFT_ORACLE_MODE=cl \
#   FREEZE_BACKBONE_ORACLE_MODE=skip \
#   bash scripts/run_libero_goal_oracle_flow_isolation_v1.sh
#
# Run only FreezeBackbone-OracleFlow:
#   SEQFT_ORACLE_MODE=skip \
#   FREEZE_BACKBONE_ORACLE_MODE=cl \
#   bash scripts/run_libero_goal_oracle_flow_isolation_v1.sh
#
# Evaluate existing chains only:
#   SEQFT_ORACLE_MODE=eval \
#   FREEZE_BACKBONE_ORACLE_MODE=eval \
#   bash scripts/run_libero_goal_oracle_flow_isolation_v1.sh
#
# Resume SeqFT-OracleFlow from CL3:
#   SEQFT_ORACLE_MODE=cl \
#   SEQFT_ORACLE_START_STAGE=3 \
#   FREEZE_BACKBONE_ORACLE_MODE=skip \
#   bash scripts/run_libero_goal_oracle_flow_isolation_v1.sh
#
# Notes
# -----
# - Periodic training checkpoints are disabled by default. Only final_model is
#   saved for every CL stage.
# - Composed checkpoints are temporary and deleted after use by default.
# - Because oracle evaluation needs multiple historical Flow heads, evaluation
#   launches multiple per-stage task-group runs and merges them into one SR matrix.
# =============================================================================
...
SEQFT_ORACLE_MODE="${SEQFT_ORACLE_MODE:-cl}"
SEQFT_ORACLE_START_STAGE="${SEQFT_ORACLE_START_STAGE:-1}"

FREEZE_BACKBONE_ORACLE_MODE="${FREEZE_BACKBONE_ORACLE_MODE:-cl}"
...
# =============================================================================
# 11. Flow-change diagnostic (report only)
# =============================================================================
...
report_flow_change() {
    ...
    python - "${reference_ckpt}" "${current_ckpt}" "${label}" <<'PY'
import sys
import torch
...
if not rkeys or rkeys != ckeys:
    raise RuntimeError(
        f"{label}: Flow key mismatch; ref={len(rkeys)} cur={len(ckeys)}"
    )
...
print(
    f"[flow-change] {label}: "
    f"checked={len(rkeys)} changed_vs_Base={changed}"
)
PY
}
...
Shared component includes:
  ...
  VLM / merger / queries / QFormer / LaWM and all other non-Flow parameters.
...
At evaluation stage CLk, all tasks use the shared parameters from CLk and
...
echo "  flow_action_query        : remains shared"
