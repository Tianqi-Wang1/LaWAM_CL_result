...
# Directly uses the existing freeze_policy.py flags; no source-code patch required.
#
# Frozen during CL:
...
#   task 9    -> CL4 Flow
#
# Execution order (normal):
#   Goal training -> Object training -> Goal eval -> Object eval
#
# Resume after the 2026-08-20 Goal-complete / Object-not-started run:
#   TRAIN_GOAL=false TRAIN_OBJECT=true MODE=train  -> Object CL1-CL4 only
#   MODE=eval                                      -> Goal+Object eval
#
# MODE:
...
for f in "${SUMMARY_SCRIPT}" "${CL_METRICS_SCRIPT}" "${FREEZE_POLICY_PY}"; do
...
# Suite-level resume controls.
# For the current resume:
#   TRAIN_GOAL=false  -> reuse the already completed Goal CL1-CL4 chain
#   TRAIN_OBJECT=true -> train Object CL1-CL4 from scratch
TRAIN_GOAL="${TRAIN_GOAL:-true}"
TRAIN_OBJECT="${TRAIN_OBJECT:-true}"

for flag_name in TRAIN_GOAL TRAIN_OBJECT; do
    flag_value="${!flag_name}"
    case "${flag_value}" in
        true|false) ;;
        *)
            echo "[ERROR] ${flag_name} must be true or false; got ${flag_value}"
            exit 1
            ;;
    esac
done
...
RUN_SUFFIX="${BATCH_TAG}_freeze_full_vlm_seqflow_v2"
...
if missing:
    raise RuntimeError(
        "Current freeze_policy.py does not match the required implementation. "
        f"Missing markers: {missing}"
    )

print("[OK] Existing freeze_policy.py already supports:")
print("     freeze_vlm_all")
print("     freeze_act_query")
print("     freeze_flow_action_query")
print("     train_flow_only")
PY
}
verify_support
...
for k, expected_value in expected.items():
    actual = fr.get(k, None)
    if actual != expected_value:
        bad.append((f"trainer.freeze.{k}", actual, expected_value))

for k in ("vlm", "action_model", "world_model"):
    actual = float(cfg.trainer.learning_rate[k].lr)
    if not math.isclose(actual, lr, rel_tol=0.0, abs_tol=1e-12):
        bad.append((f"trainer.learning_rate.{k}.lr", actual, lr))
...
if bad:
    ...
    raise RuntimeError(f"{label} config mismatch: {details}")

print(f"[OK] {label}: strict full-VLM/interface freeze config verified.")
print("     freeze_vlm_all=True")
print("     freeze_act_query=True")
print("     freeze_flow_action_query=True")
print("     train_flow_only=False")
print("     unfreeze_lam_decoder=True")
PY
}
...
train_suite() {
    ...
    echo "# TRAIN ${suite}"
    echo "################################################################################"
    echo "Frozen    : freeze_vlm_all + act_query + flow_action_query"
    ...
    echo "Flow-only : false (QFormer/LaWM remain trainable)"
    ...
    # Clear only CUDA visibility between suites.
    # IMPORTANT: keep the shell variable NUM_PROCESSES alive; the previous v2
    # unset it here, which caused Object training to abort with:
    #   NUM_PROCESSES: unbound variable
...
echo "=========================================================="
echo "MODE           : ${MODE}"
echo "TRAIN_GOAL     : ${TRAIN_GOAL}"
echo "TRAIN_OBJECT   : ${TRAIN_OBJECT}"
...
if [ "${MODE}" = "all" ] || [ "${MODE}" = "train" ]; then
    if [ "${TRAIN_GOAL}" = "true" ]; then
        ...
        echo
        echo "[SKIP TRAIN] libero_goal"
        echo "             Reusing existing Goal CL1-CL4 checkpoints."
    fi
    ...
    if [ "${TRAIN_OBJECT}" = "true" ]; then
        ...
        echo
        echo "[SKIP TRAIN] libero_object"
        echo "             Reusing existing Object CL1-CL4 checkpoints."
    fi
fi
