...
# Fresh single-task sweep over the CL sequence tasks.
# IMPORTANT: every T6/T7/T8/T9 model starts independently from the SAME Formal Base.
# This is NOT sequential checkpoint chaining. It measures per-task plasticity/FWT.
...
case "${TASK_ID}" in
  6|7|8|9) ;;
  *) echo "[ERROR] TASK_ID must be one of 6,7,8,9"; exit 2 ;;
esac
STAGE=$((TASK_ID - 5))
...
TRAIN_EVAL_INTERVAL="${TRAIN_EVAL_INTERVAL:-1000}"
...
case "${MODE}" in
  ...
  b3)
    ...
    MODE_OFFSET=300
    ;;
  *) echo "[ERROR] MODE must be b1, b2, or b3"; exit 2 ;;
esac

if [ "${CONDITIONING_BOTTLENECK}" != "128" ] || [ "${NONLINEAR_BOTTLENECK}" != "128" ]; then
  echo "[ERROR] v9 exact budgets/verifier require CONDITIONING_BOTTLENECK=128 and NONLINEAR_BOTTLENECK=128"
  exit 2
fi
...
if missing: raise RuntimeError(f'Missing nonlinear-action support: {missing}')
print('[OK] Nonlinear-action support detected.')
PYCODE

EXPERIMENT_ROOT="/home/jincai_guo/tianqi/CVPR2027/checkpoints/lawam_cl/libero_goal/seq_single_task_nonlinear_10k_v10/${MODE}/task${TASK_ID}"
RUN_ROOT="${EXPERIMENT_ROOT}/runs"; LOG_ROOT="${EXPERIMENT_ROOT}/logs"
OUTPUT_ROOT="${ROOT}/results/eval_runs/lawam_cl/libero_goal/seq_single_task_nonlinear_10k_v10/${MODE}/task${TASK_ID}"
MERGE_SCRIPT="${ROOT}/scripts/merge_lora_checkpoint_v6.py"
VERIFY_SCRIPT="${ROOT}/scripts/verify_seq_single_nonlinear_v10.py"
...
echo "=========================================================="
echo " Seq-task single-task sweep: CL${STAGE}/T${TASK_ID}"
echo " ${LABEL}"
echo "=========================================================="
echo "Protocol                  : FRESH from Formal Base (NO stage chaining)"
echo "Base                      : ${BASE_CKPT}"
echo "Task                      : T${TASK_ID} / CL${STAGE}"
...
export MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-$((31000 + MODE_OFFSET + TASK_ID))}"
...
verify_run "${LABEL} / T${TASK_ID}" "${RUN}"
...
# Guard against accidentally training the wrong CL task.
python - "${RUN}/config.yaml" "${BASE_CKPT}" "${TASK_ID}" <<'PYCODE'
import sys
from omegaconf import OmegaConf
cfg=OmegaConf.load(sys.argv[1]); base=sys.argv[2]; task=int(sys.argv[3])
if str(cfg.trainer.pretrained_checkpoint) != base:
    raise RuntimeError('Wrong Base checkpoint')
if list(cfg.datasets.vla_data.cl_task_ids) != [task]:
    raise RuntimeError(f'Wrong task filter: {cfg.datasets.vla_data.cl_task_ids}, expected={[task]}')
print(f'[OK] Fresh Base + single-task T{task} protocol verified.')
PYCODE
...
if [ "${DO_EVAL,,}" = "true" ]; then
  ...
  python "${SUMMARY_SCRIPT}" --run-dir "${SUITE_DIR}" --task-ids "${TASK_ID}" --expected-trials "${NUM_TRIALS}"
  echo "${SUITE_DIR}/per_task_summary.csv" > "${EXPERIMENT_ROOT}/latest_summary_path.txt"
fi

cat > "${EXPERIMENT_ROOT}/PROTOCOL.txt" <<EOF2
Mode: ${MODE}
Task: T${TASK_ID} (CL${STAGE})
Initialization: Formal Base checkpoint; independent fresh single-task adaptation.
No parameters/checkpoint are inherited from T$((TASK_ID-1)) or any previous CL stage.
Purpose: measure current-task plasticity/FWT on each task in the sequential task set T6-T9.
Expected task-specific parameter budget: ${EXPECTED_PARAMS}
EOF2

echo "=========================================================="
echo " COMPLETE: ${MODE^^} / T${TASK_ID}"
echo " Run       : ${RUN}"
