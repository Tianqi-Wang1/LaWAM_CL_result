...
# =============================================================================
# R-Pretrain: LIBERO-Goal T9
#             Base upstream + FROZEN PRETRAIN Flow + Residual Expert
#             Main checkpoint is still Base; ONLY the Flow core is overridden
#             from the pretraining checkpoint. Default residual capacity: R4.
#
# Parallel-friendly: auto-preserve global batch=256, dedicated DDP port 29731,
...
EXPERIMENT_ROOT="/home/jincai_guo/tianqi/CVPR2027/checkpoints/lawam_cl/libero_goal/t9_residual_expert_split_v1/residual_pretrainflow"
RUN_ROOT="${EXPERIMENT_ROOT}/runs"
LOG_ROOT="${EXPERIMENT_ROOT}/logs"
OUTPUT_ROOT="${ROOT}/results/eval_runs/lawam_cl/libero_goal/t9_residual_expert_split_v1/residual_pretrainflow"
...
export MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-29731}"
...
MASTER_LOG="${LOG_ROOT}/residual_r${RESIDUAL_BLOCKS}_pretrainflow_${PIPELINE_TIMESTAMP}.log"
exec > >(tee -a "${MASTER_LOG}") 2>&1
RUN_ID="t9_residual_r${RESIDUAL_BLOCKS}_pretrainflow_${MAX_TRAIN_STEPS}step_gbs${GLOBAL_BATCH_SIZE}"

echo "=========================================================="
echo " R-Pretrain: Frozen Pretrain Flow + R${RESIDUAL_BLOCKS} -> T9"
echo "=========================================================="
echo "Base upstream   : ${BASE_CKPT}"
echo "Frozen Flow     : ${PRETRAIN_CKPT}"
...
if [ "${DO_TRAIN}" = "true" ]; then
    ...
    bash train_lawam.sh \
        ...
        "--trainer.policy_flow_override_checkpoint=${PRETRAIN_CKPT}" \
...
verify_run "Residual R${RESIDUAL_BLOCKS} Pretrain-Flow" "${RUN}"
verify_stats "${BASE_STATS}" "${RUN}/dataset_statistics.json"
python "${VERIFY_SCRIPT}" \
    --mode residual \
    --base-ckpt "${BASE_CKPT}" \
    --flow-source-ckpt "${PRETRAIN_CKPT}" \
...
if [ "${DO_EVAL}" = "true" ]; then
    ...
    ALIAS="residual_r${RESIDUAL_BLOCKS}_pretrainflow"
