...
# Evaluation switches.  These are independent from training switches so that
# MODE=all can train/evaluate only one suite.
EVAL_GOAL="${EVAL_GOAL:-true}"
EVAL_OBJECT="${EVAL_OBJECT:-true}"

GOAL_START_STAGE="${GOAL_START_STAGE:-1}"
OBJECT_START_STAGE="${OBJECT_START_STAGE:-1}"

for flag_name in TRAIN_GOAL TRAIN_OBJECT EVAL_GOAL EVAL_OBJECT; do
    flag_value="${!flag_name}"
    case "${flag_value}" in
        true|false) ;;
        *)
            echo "[ERROR] ${flag_name} must be true/false; got ${flag_value}"
            exit 1
            ;;
    esac
done
...
# Evaluate only the requested suites.
if [ "${MODE}" = "all" ] || [ "${MODE}" = "eval" ]; then
    if [ "${EVAL_GOAL}" = "true" ]; then
        evaluate_suite "libero_goal" "${GOAL_BASE}" "${GOAL_RUN_ROOT}" "${GOAL_OUTPUT_ROOT}"
    fi

    if [ "${EVAL_GOAL}" = "true" ] && [ "${EVAL_OBJECT}" = "true" ]; then
        sleep 5
    fi

    if [ "${EVAL_OBJECT}" = "true" ]; then
