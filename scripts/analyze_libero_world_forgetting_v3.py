...
def load_cache(path: Path):
    """Load analysis cache files without mmap.

    The latent-action helper intentionally uses mmap=True for large checkpoint/cache
    reads.  World analysis also writes derived cache files; using normal CPU-backed
    tensors here avoids SIGBUS hazards from file-backed storages.
    """
    try:
...
def add_base_policy_predictions(
    *,
    ...
    """Compute the actual Base-policy world prediction without rewriting world_reference.

    Important:
    `analyze_libero_latent_action_forgetting.load_state()` uses mmap=True.  Rewriting
    a file while tensors are still backed by that same mmap can trigger SIGBUS.
    Therefore this function treats `world_reference/` as immutable and writes
    `base_policy_pred` into a completely separate cache tree.
    """
    if out_root.exists():
        shutil.rmtree(out_root)
    ...
    for task in task_ids:
        ...
        for bi, fixed_path in enumerate(task_batches[int(task)]):
            if bi == 0:
                print(f"[Base] task={task}: entering first Base-policy batch (non-mmap caches)")
            # These are small analysis cache files: deliberately do NOT mmap them.
            fixed = load_cache(fixed_path)
            ...
            out_path = out_root / f"task_{task}" / f"batch_{bi:04d}.pt"
            out_path.parent.mkdir(parents=True, exist_ok=True)
            torch.save(
                {
                    "z_bb": z_bb.detach().cpu().float().contiguous(),
                    "base_policy_pred": (
                        base_policy_pred.detach()
                        .cpu()
                        .to(torch.bfloat16)
                        .contiguous()
                    ),
...
def main():
    ...
    run_meta = {
        ...
        "references": {
            ...
            "base_policy": "W_Base(h_t, z_BB), stored in a separate immutable cache and used as the actual Base-pipeline normalization",
        },
        "aggregation": (
    ...
    base_policy_root = args.output_dir / "base_policy_reference"
    ...
    for task in args.task_ids:
        ...
        for bi, _ in enumerate(task_batches[int(task)]):
            ...
            base_policy = load_cache(
                base_policy_root / f"task_{task}" / f"batch_{bi:04d}.pt"
            )
            ref = dict(ref)
            ref["base_policy_pred"] = base_policy["base_policy_pred"]
            routes = {
