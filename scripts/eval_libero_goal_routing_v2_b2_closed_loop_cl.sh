...
OUT_ROOT="${ROOT}/results/eval_runs/lawam_cl/libero_goal/routing_v2_b2_closed_loop_cl/delta_${DELTA_TAG}/${STAMP}"
mkdir -p "${OUT_ROOT}"
...
Routing-V2 B2 formal CLOSED-LOOP CL-only evaluation.
...
Semantic gate: C_sem < ${GATE_THRESHOLD}.
Adaptive fusion: lambda_max=${LAMBDA_MAX}, gamma=${FUSION_GAMMA}.
Selected Skill executes with its complete task-specific path including its LaWM-LoRA.
NUM_TRIALS=${NUM_TRIALS}
EOF

echo "======================================================================"
echo " Routing-V2 B2 FORMAL CLOSED-LOOP CL evaluation"
...
POINTER="${V2_ROOT}/latest_b2_closed_loop_delta_${DELTA_TAG}.txt"
echo "${OUT_ROOT}" > "${POINTER}"
echo "${OUT_ROOT}" > "${V2_ROOT}/latest_b2_closed_loop_cl_run.txt"
echo "======================================================================"
echo " Routing-V2 B2 CLOSED-LOOP CL evaluation COMPLETE"
echo " Output  : ${OUT_ROOT}"
echo " Matrix  : ${MATRIX}"
