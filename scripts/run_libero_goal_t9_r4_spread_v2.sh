...
# =============================================================================
# E1: T9 R4-SPREAD
# Base upstream + frozen Base Flow + 4 nonlinear residual experts at DiT layers
# [0, 5, 10, 15]. Only residual experts are trainable.
...
EXPERIMENT_ROOT="/home/jincai_guo/tianqi/CVPR2027/checkpoints/lawam_cl/libero_goal/t9_residual_followup_v2/r4_spread"
RUN_ROOT="${EXPERIMENT_ROOT}/runs"
LOG_ROOT="${EXPERIMENT_ROOT}/logs"
OUTPUT_ROOT="${ROOT}/results/eval_runs/lawam_cl/libero_goal/t9_residual_followup_v2/r4_spread"
...
RESIDUAL_LAYER_INDICES="[0,5,10,15]"
...
for key in ("residual_expert_layer_indices","residual_expert_num_blocks"):
    if key not in cf: raise RuntimeError(f"Missing flow config support: {key}")
if "train_flow_residual_expert" not in ff:
    raise RuntimeError("Missing residual training support")
print("[OK] R4-Spread code support detected.")
PY

STAMP=$(date +"%Y%m%d_%H%M%S")
RUN_ID="t9_r4_spread_0_5_10_15_${MAX_TRAIN_STEPS}step_4gpu_bs${PER_DEVICE_BATCH_SIZE}_ga${GRADIENT_ACCUMULATION_STEPS}"
MASTER_LOG="${LOG_ROOT}/${RUN_ID}_${STAMP}.log"
exec > >(tee -a "${MASTER_LOG}") 2>&1

echo "=========================================================="
echo " T9 R4-SPREAD: Base Flow + residual layers 0,5,10,15"
echo "=========================================================="
echo "Base          : ${BASE_CKPT}"
...
echo "LR            : ${ACTION_LR}"
echo "Residual      : ${RESIDUAL_LAYER_INDICES}"
...
export MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-29831}"
...
bash train_lawam.sh \
    ...
    "--framework.action_model.flow_cfg.residual_expert_layer_indices=${RESIDUAL_LAYER_INDICES}" \
...
python - "${RUN}/config.yaml" "${BASE_CKPT}" <<'PY'
import sys
...
if bool(fr.train_flow_only) or bool(fr.train_flow_lora) or not bool(fr.train_flow_residual_expert): raise RuntimeError("Wrong training mode")
if int(fc.residual_expert_num_blocks) != 0: raise RuntimeError("R4-Spread must use explicit placement")
if list(fc.residual_expert_layer_indices) != [0,5,10,15]: raise RuntimeError(f"Wrong residual placement: {fc.residual_expert_layer_indices}")
print("[OK] R4-Spread config verified.")
PY
