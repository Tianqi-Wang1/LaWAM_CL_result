...
# =============================================================================
# E2: T9 R8-LAST capacity control
# Base upstream + frozen Base Flow + residual experts on final 8 DiT blocks
# [8,9,10,11,12,13,14,15]. Only residual experts are trainable.
...
EXPERIMENT_ROOT="/home/jincai_guo/tianqi/CVPR2027/checkpoints/lawam_cl/libero_goal/t9_residual_followup_v2/r8_last"
RUN_ROOT="${EXPERIMENT_ROOT}/runs"
LOG_ROOT="${EXPERIMENT_ROOT}/logs"
OUTPUT_ROOT="${ROOT}/results/eval_runs/lawam_cl/libero_goal/t9_residual_followup_v2/r8_last"
...
if "train_flow_residual_expert" not in ff: raise RuntimeError("Missing residual training support")
print("[OK] R8-Last code support detected.")
PY

STAMP=$(date +"%Y%m%d_%H%M%S")
RUN_ID="t9_r8_last_${MAX_TRAIN_STEPS}step_4gpu_bs${PER_DEVICE_BATCH_SIZE}_ga${GRADIENT_ACCUMULATION_STEPS}"
MASTER_LOG="${LOG_ROOT}/${RUN_ID}_${STAMP}.log"
exec > >(tee -a "${MASTER_LOG}") 2>&1

echo "=========================================================="
echo " T9 R8-LAST: Base Flow + final 8 residual experts"
echo "=========================================================="
echo "Base          : ${BASE_CKPT}"
...
echo "LR            : ${ACTION_LR}"
echo "Residual      : final 8 layers [8..15]"
...
export MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-29832}"
...
bash train_lawam.sh \
    ...
    "--framework.action_model.flow_cfg.residual_expert_num_blocks=8" \
...
if int(fc.residual_expert_num_blocks) != 8: raise RuntimeError("Wrong residual block count")
if fc.residual_expert_layer_indices is not None: raise RuntimeError("R8-Last must use final-N placement")
print("[OK] R8-Last config verified.")
PY
