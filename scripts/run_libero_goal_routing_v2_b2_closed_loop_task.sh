...
Routing-V2 B2 CLOSED-LOOP / ${STAGE} / true LIBERO task T${TASK_ID}
...
B2 routing definition:
  ...
  Fusion: S=(1-lambda(C))*Semantic + lambda(C)*Dynamics.
...
echo "======================================================================"
echo " Routing-V2 B2 CLOSED-LOOP ${STAGE} / true task T${TASK_ID}"
...
CUDA_VISIBLE_DEVICES="${POLICY_GPU}" "${STAR_VLA_PYTHON}" \
  deployment/model_server/server_policy_routing_v2_b2_closed_loop.py \
...
while true; do
  if ! kill -0 "${SERVER_PID}" 2>/dev/null; then
    echo "[ERROR] Routing-V2 B2 server exited before becoming ready"
  ...
  if [ $((now - SERVER_READY_START)) -ge "${SERVER_READY_TIMEOUT}" ]; then
    echo "[ERROR] Timed out waiting for Routing-V2 B2 server on port ${PORT}"
    tail -n 220 "${SERVER_LOG}" || true
    exit 1
  fi
  sleep 1
done
echo "[OK] Routing-V2 B2 server ready on port ${PORT}"
...
echo "[OK] ${STAGE}/T${TASK_ID} B2 complete"
