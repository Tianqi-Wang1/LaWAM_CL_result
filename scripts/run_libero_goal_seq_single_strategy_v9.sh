...
RESULT_ROOT="/home/jincai_guo/tianqi/CVPR2027/checkpoints/lawam_cl/libero_goal/seq_single_task_nonlinear_v9"
cd "${ROOT}"
...
for task in ${TASKS}; do
  echo
  echo "#################### ${MODE^^} / T${task} ####################"
  MODE="${MODE}" TASK_ID="${task}" bash scripts/run_libero_goal_seq_single_variant_v9.sh
  echo "################## DONE ${MODE^^} / T${task} ##################"
done

python scripts/aggregate_seq_single_task_v9.py \
  --mode "${MODE}" \
  --root "${RESULT_ROOT}" \
  --tasks ${TASKS}
