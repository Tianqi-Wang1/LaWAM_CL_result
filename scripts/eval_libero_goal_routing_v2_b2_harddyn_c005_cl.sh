...
DELTA_TAG="${GATE_THRESHOLD/./p}"
OUT_ROOT="${ROOT}/results/eval_runs/lawam_cl/libero_goal/routing_v2_b2_harddyn_c005_closed_loop_cl/${STAMP}"
mkdir -p "${OUT_ROOT}"
...
for t in 6 7 8 9; do
  ...
  [ -f "${V2_ROOT}/task${t}/routing_memory_variants/basewm_hdh/dynamics_ae.pt" ] || { echo "[ERROR] missing B2 Dynamics memory T${t}"; exit 1; }
done

cat > "${OUT_ROOT}/PROTOCOL.txt" <<EOF
Routing-V2 B2 HARD-DYNAMICS formal CLOSED-LOOP CL-only evaluation.
B2 = Shared Base-WM + [h_t, Delta h] Dynamics verifier.
...
Decision rule: C_sem < ${GATE_THRESHOLD} -> choose entirely by B2 Dynamics reconstruction error within Semantic Top-2; otherwise choose Semantic Top-1.
No weighted Semantic/Dynamics fusion is used inside the gate.
Selected Skill executes with its complete task-specific path including its LaWM-LoRA.
NUM_TRIALS=${NUM_TRIALS}
EOF

echo "======================================================================"
echo " Routing-V2 B2 HARD-DYNAMICS FORMAL CLOSED-LOOP CL evaluation"
echo " B2           : Shared Base-WM + [h_t, Delta h]"
echo " Decision     : C_sem < ${GATE_THRESHOLD} -> HARD Dynamics; else Semantic Top-1"
...
run_stage() {
  ...
  for task in ${tasks}; do
    ...
    bash scripts/run_libero_goal_routing_v2_b2_harddyn_task.sh
    offset=$((offset+1))
  done
...
run_stage CL1 "6"       "6"       7200
run_stage CL2 "6 7"     "6 7"     7300
run_stage CL3 "6 7 8"   "6 7 8"   7400
run_stage CL4 "6 7 8 9" "6 7 8 9" 7500
...
print('[RoutingV2][B2] all-stage routing summary')
print(out.read_text(encoding='utf-8').rstrip())
PY

POINTER="${V2_ROOT}/latest_b2_harddyn_c005_closed_loop.txt"
echo "${OUT_ROOT}" > "${POINTER}"
echo "${OUT_ROOT}" > "${V2_ROOT}/latest_b2_harddyn_closed_loop_cl_run.txt"
echo "======================================================================"
echo " Routing-V2 B2 HARD-DYNAMICS CLOSED-LOOP CL evaluation COMPLETE"
echo " Output  : ${OUT_ROOT}"
echo " Matrix  : ${MATRIX}"
