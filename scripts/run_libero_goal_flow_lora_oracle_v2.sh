...
# =============================================================================
# LaWAM LIBERO-Goal: Frozen WAM + Independent Flow-LoRA Oracle v2
#
# Purpose:
...
#
# v2 adds: non-contiguous stage selection, LR-aware run IDs, and visible audits.
#
...
LORA_ALPHA_TAG=$(sanitize_tag "${LORA_ALPHA}")
LORA_LR_TAG=$(sanitize_tag "${LORA_LR}")
FLOW_LORA_TAG="${FLOW_LORA_TAG:-r${LORA_RANK}_a${LORA_ALPHA_TAG}_lr${LORA_LR_TAG}}"
LORA_SCALE=$(awk -v a="${LORA_ALPHA}" -v r="${LORA_RANK}" 'BEGIN {printf "%.8g", a/r}')

EXPERIMENT_ROOT="/home/jincai_guo/tianqi/CVPR2027/checkpoints/lawam_cl/libero_goal/flow_lora_oracle/${FLOW_LORA_TAG}"
RUN_ROOT="${EXPERIMENT_ROOT}/heads"
LOG_ROOT="${EXPERIMENT_ROOT}/logs"
OUTPUT_ROOT="${ROOT}/results/eval_runs/lawam_cl/libero_goal/flow_lora_oracle/${FLOW_LORA_TAG}"
...
# Optional non-contiguous selection, e.g. FLOW_LORA_STAGES="1 4".
FLOW_LORA_STAGES="${FLOW_LORA_STAGES:-}"

case "${FLOW_LORA_MODE}" in cl|eval) ;; *) echo "[ERROR] FLOW_LORA_MODE must be cl or eval"; exit 1 ;; esac

if [ -n "${FLOW_LORA_STAGES}" ]; then
    read -r -a STAGES <<< "${FLOW_LORA_STAGES}"
else
    ...
    mapfile -t STAGES < <(seq "${FLOW_LORA_START_STAGE}" "${FLOW_LORA_END_STAGE}")
fi

[ "${#STAGES[@]}" -gt 0 ] || { echo "[ERROR] No stages selected."; exit 1; }
for stage in "${STAGES[@]}"; do
    if ! [[ "${stage}" =~ ^[1-4]$ ]]; then
        echo "[ERROR] Invalid stage in FLOW_LORA_STAGES: ${stage}"; exit 1
    fi
done
...
verify_config() {
    local path="$1" task="$2"
python - "${path}" "${task}" "${BASE_CKPT}" "${LORA_RANK}" "${LORA_ALPHA}" "${LORA_DROPOUT}" "${LORA_LR}" <<'PY'
import math, sys
...
expected_params = 290816 * expected_rank
if params != expected_params:
    raise RuntimeError(
        f"{label}: canonical LoRA params={params}, expected={expected_params} for rank={expected_rank}"
    )
if any(int(cur[k].shape[0]) != expected_rank for k in canon_A):
    raise RuntimeError(f"{label}: LoRA rank mismatch")
...
run_id_for_stage() {
    ...
    echo "cl${stage}_t${task}_${MAX_TRAIN_STEPS}step_${NUM_TRAIN_GPUS}gpu_bs${PER_DEVICE_BATCH_SIZE}_ga${GRADIENT_ACCUMULATION_STEPS}_flow_lora_r${LORA_RANK}_a${LORA_ALPHA_TAG}_lr${LORA_LR_TAG}_from_base"
}
...
train_one_stage() {
    ...
    echo " Base: ${BASE_CKPT}"
    echo " LoRA: r=${LORA_RANK}, alpha=${LORA_ALPHA}, lr=${LORA_LR}, dropout=${LORA_DROPOUT}"
    echo " Targets: Attention Q/K/V/O + FFN in all 16 Flow blocks"
    echo " Trainable expected      : $((290816 * LORA_RANK)) params"
    echo "=========================================================="
...
evaluate_one_stage() {
    ...
    local alias="flow_lora_${FLOW_LORA_TAG}_${label}_T${task}"
...
MASTER_LOG="${LOG_ROOT}/flow_lora_${FLOW_LORA_TAG}_${PIPELINE_TIMESTAMP}.log"
...
echo "LoRA rank/alpha  : ${LORA_RANK}/${LORA_ALPHA} (scale=${LORA_SCALE})"
echo "LoRA action LR   : ${LORA_LR}"
echo "LoRA dropout     : ${LORA_DROPOUT}"
echo "Target           : Attention Q/K/V/O + FFN"
echo "Mode/stages      : ${FLOW_LORA_MODE} / ${STAGES[*]}"
echo "Experiment tag   : ${FLOW_LORA_TAG}"
...
if [ "${FLOW_LORA_MODE}" = "cl" ]; then
    ...
    for stage in "${STAGES[@]}"; do
        ...
        # Keep the full hard-audit output visible in the master log.
        prepare_stage_artifacts "${stage}"
    done
    unset CUDA_VISIBLE_DEVICES || true
    unset NUM_PROCESSES || true
else
    # In eval mode, still audit/merge each existing adapter before rollout.
    for stage in "${STAGES[@]}"; do
        prepare_stage_artifacts "${stage}"
    done
fi
...
for stage in "${STAGES[@]}"; do
    run=$(resolve_stage_run "${stage}")
    verify_run "Flow-LoRA CL${stage}/T$((stage+5))" "${run}"
